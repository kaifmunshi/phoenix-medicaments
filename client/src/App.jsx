...
import { fetchCertificates, fetchProducts, sendContactEnquiry } from "./api";
...
function CategoriesPage() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      Object.entries(productGroups).map(([key, group]) => (
        fetchProducts({ type: group.label, page: 1, limit: 1 })
          .then((data) => [key, Number(data.total || 0)])
          .catch(() => [key, 0])
      ))
    ).then((entries) => {
      if (isCurrent) setCategoryCounts(Object.fromEntries(entries));
    });
  ...
  return (
    <section className="page-shell product-page">
      ...
      {isLoadingProducts && (
        <LoadingBanner title={`Preparing ${currentGroup.label}`} text="Sourcing the latest active products from the catalogue." />
      )}
